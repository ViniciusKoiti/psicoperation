...
class TaskUpdateRequest {
  ...
  @override
  String toString() => 'TaskUpdateRequest[title=$title, dueDate=$dueDate, completedAt=$completedAt]';
  ...
  static List<TaskUpdateRequest> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <TaskUpdateRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TaskUpdateRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TaskUpdateRequest> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = TaskUpdateRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<TaskUpdateRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TaskUpdateRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = TaskUpdateRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
